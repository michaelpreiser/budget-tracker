...
export default function Dashboard({ transactions }: Props) {
  ...
  for (const t of transactions) {
    if (t.type === 'expense') {
      ...
      savMap[t.category] = (savMap[t.category] ?? 0) + t.amount
    }
  }

  const totalExpenses = Object.values(expMap).reduce((s, v) => s + v, 0)
  const totalSavings = Object.values(savMap).reduce((s, v) => s + v, 0)
  ...
  const savingsCategories = Object.entries(savMap).sort((a, b) => b[1] - a[1])

  const hasAny = expenseCategories.length > 0 || incomeCategories.length > 0 || savingsCategories.length > 0
  ...
  return (
    <div className="bg-slate-900 border border-slate-700/50 rounded-2xl p-5 shadow-xl space-y-5">
      ...
      {expenseCategories.length > 0 && (
        <div>
          <p className="text-slate-500 text-xs font-medium uppercase tracking-wider mb-3">Expenses</p>
                ...
                <div key={cat}>
                  ...
                  >
                    ...
                    <div className="flex items-baseline gap-1.5 flex-shrink-0">
                      <span className="text-red-400 text-sm font-medium tabular-nums">−${fmt(amt)}</span>
      ...
      {savingsCategories.length > 0 && (
        <div>
          <p className="text-slate-500 text-xs font-medium uppercase tracking-wider mb-3">Savings</p>
          <div className="space-y-2">
            {savingsCategories.map(([cat, amt]) => {
              ...
              return (
                <div key={cat}>
                  ...
                  >
                    ...
                    <div className="flex items-baseline gap-1.5 flex-shrink-0">
                      <span className="text-blue-400 text-sm font-medium tabular-nums">+${fmt(amt)}</span>
                      <span className="text-slate-600 text-xs tabular-nums">{pct.toFixed(0)}%</span>
                    </div>
                  </button>
                  <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden mb-1">
      ...
      {incomeCategories.length > 1 && (
        <div>
          <p className="text-slate-500 text-xs font-medium uppercase tracking-wider mb-3">Income Sources</p>
          <div className="space-y-2">

      ...
      <AppHeader
        navigator={
          <div className="flex items-center gap-0.5 bg-slate-900 border border-slate-800 rounded-xl px-1 py-0.5">
            <button onClick={() => setSelectedMonth((m) => shiftMonth(m, -1))} className="p-1.5 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-slate-800 transition-colors text-sm leading-none">‹</button>
            <span className="text-slate-200 font-medium text-xs min-w-[130px] text-center tabular-nums">{fmtMonthLabel(selectedMonth)}</span>
            <button onClick={() => setSelectedMonth((m) => shiftMonth(m, 1))} className="p-1.5 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-slate-800 transition-colors text-sm leading-none">›</button>
          </div>
        }
      />
